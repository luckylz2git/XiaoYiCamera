...
def ConnectCam():
  print "%s.%s: Open Camera Port 8787" %(time.strftime("%H:%M:%S"), ("%0.3f" %time.time()).split(".")[1])
  ...
  while True:
    i += 1
    srvopen = srv.connect_ex(('192.168.42.121', 8787))
    if srvopen == 0:
      print "%s.%s: Success Camera Port 8787" %(time.strftime("%H:%M:%S"), ("%0.3f" %time.time()).split(".")[1])
      srv.setblocking(0)
      return srv
    else:
      print "%s.%s: Failure Camera Port 8787 %d" %(time.strftime("%H:%M:%S"), ("%0.3f" %time.time()).split(".")[1], srvopen)
    time.sleep(10)

def ConnectPhone(cam):
  print "%s.%s: Open Phone Port 8787" %(time.strftime("%H:%M:%S"), ("%0.3f" %time.time()).split(".")[1])
  try:
    srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    srv.bind(('192.168.42.1', 8787))
    srv.listen(5)
  except:
    print "%s.%s: Failure Phone Port 8787" %(time.strftime("%H:%M:%S"), ("%0.3f" %time.time()).split(".")[1])
    return
  #srv.setblocking(0)
