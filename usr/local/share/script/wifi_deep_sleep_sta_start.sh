...
WPA_SCAN ()
{
	/usr/bin/wpa_supplicant -D${driver} -iwlan0 -C /var/run/wpa_supplicant -B
	/usr/bin/wpa_cli scan
	sleep 3
	scan_result=`/usr/bin/wpa_cli scan_r`
...
WPA_GO ()
{
	...
	/usr/bin/wpa_supplicant -D${driver} -iwlan0 -c/tmp/wpa_supplicant.conf -B
