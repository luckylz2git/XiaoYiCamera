...
#fCNT=$#
#echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ count = ${fCNT}  arg[1] = ${1}  arg[2] = ${2}  arg[3] = ${3}  arg[4] = ${4}"
#
#if [ "${1}" != "" ]; then
#	usecache=${1}
#fi
...
hostapd_conf()
{
	if [ "${usecache}" != "" ] && [ -e /tmp/hostapd.conf ]; then
		...
		if [ ${rval} -eq 0 ]; then
			...
			echo "wpa_group_rekey=0" >> /tmp/hostapd.conf
			echo "wpa_strict_rekey=0" >> /tmp/hostapd.conf
			echo "wpa_gmk_rekey=0" >> /tmp/hostapd.conf
			echo "wpa_ptk_rekey=0" >> /tmp/hostapd.conf
			echo "ap_max_inactivity=300" >> /tmp/hostapd.conf
...
apply_ap_conf()
{
	...
	#	rval=$?
		...
		rval1=$?
		HostapdName="hostapd"
		#echo " @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ DEBUG >> 001 >> rval0 = ${rval0}   rval1 = ${rval1} "
		if [ ${rval0} -eq 0 ] || [ ${rval1} -eq 0 ]; then
		#echo " @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ DEBUG >> 002 >> rval0 = ${rval0}   rval1 = ${rval1} "
			HostapdName="hostapd_autochannel_retartchip"
		fi
		
		#echo " @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ DEBUG >> 003 >> HostapdName = ${HostapdName} "
		...
		echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ AP Manager >> ${HostapdName}"
			...
			if [ ! -e /tmp/fuse_d/BCM4330_DEBUG/do_debug ]; then
				${HostapdName} -B -P /var/run/hostapd/hostapd.pid /tmp/hostapd.conf
			else
				${HostapdName} -B -ddddd -t -f /tmp/fuse_d/BCM4330_DEBUG/hostapd_debug.log -P /var/run/hostapd/hostapd.pid /tmp/hostapd.conf
			fi
	...
	#add by jarry.jin 2015-05.14, dump kmesg
	/usr/local/share/script/do_bcm4330_debug.sh
